...
<template>
    ...
    <section class="agro2">
         <img src="/public/Banner3Agro2.png" alt="" width="1834" height="">

         <div>
    <ul>
      <li v-for="produto in agro2" :key="produto.id">
        <p><img :src="produto.imagem" alt="" width="200" height="200"></p>
        <p class="imagem">{{ imagem }}</p>
        <p class="nome"> {{ produto.nome }}</p>

        <p v-for="preco in agro2" :key="preco.id"></p>
        <p class="preco"> R${{ produto.preco }}</p>

        <p v-for="local in agro2" :key="local.id"></p>
...
section.agro2 div p.nome {
  margin: 0 0 0.2vw 0;
  font-family: bold;
  font-size: 1.6vw;
}
section.agro2 div p.local {
  font-family: bold;
  font-size: 1.4vw;
}
section.agro2 div p.preco {
  font-family: bold;
  font-size: 1.2vw;
}
section.agro2 div button.comprar {
  font-size: 1vw;
  font-weight: bold;
  color: white;
  margin: 0.6vw 0 0 0;
