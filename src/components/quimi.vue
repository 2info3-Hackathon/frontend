...
<template>
    ...
    <section class="quimi">
        <img src="/public/3Quimi.png" alt="" width="1834" height="">

         <div>
    <ul>
      <li v-for="produto in quimi" :key="produto.id">
        <p><img :src="produto.imagem" alt="" width="200" height="200"></p>
        <p class="imagem">{{ imagem }}</p>
        <p class="nome"> {{ produto.nome }}</p>

        <p v-for="preco in quimi" :key="preco.id"></p>
        <p class="preco"> R${{ produto.preco }}</p>

        <p v-for="local in quimi" :key="local.id"></p>
...
section.quimi div p.nome {
  margin: 0 0 0.2vw 0;
  font-family: bold;
  font-size: 1.6vw;
}
section.quimi div p.local {
  font-family: bold;
  font-size: 1.4vw;
}
section.quimi div p.preco {
  font-family: bold;
  font-size: 1.2vw;
}
section.quimi div button.comprar {
  font-size: 1vw;
  font-weight: bold;
  color: white;
  margin: 0.6vw 0 0 0;
...
section.quimi div button.comprar:hover {
